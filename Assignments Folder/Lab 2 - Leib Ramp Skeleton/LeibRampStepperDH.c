...
int main()
{
    ...
    while(true)
    {
        ...
        accelSteps = (pow(maxPermissSpeed, 2) - pow(minSpeed, 2))/(2*maxAccel);                                 // calculate number of steps to accelerate over
        ...
        if (2 * accelSteps > stepsToGo)
        {
            ...
            maxSpeed = sqrt(pow(minSpeed, 2) + maxAccel*2*accelSteps);                                          // calculate new max speed for motor
            long accelSteps = (accelSteps/2);                                                                   // steps to accelerate over now halved, convert to long to avoid overflow
...
/* Calcuate new value of step interval p based on constants defined in loop() */
{
  ...
  {
    /* Equation (9) */
    m = -R;                                                                                                     // m is a negative constant multiplier
  ...
  {
    /* Equation 10 */
    m = R;                                                                                                      // m is a positive constant multiplier
  ...
  {
    /* Equation (11)	*/
    m = 0;                                                                                                      // m is has no value (is not a multiplier)
       
  }
  
  // STEP 6b, c and d using Equations (12) and (13)
  q = m*p*p;                                                                                                    // define q as a convenient condensed equation
  p = p*(1 + q + 1.5*q*q);                                                                                      // update p based on existing p and value of q
